	...
	def _inject_panelexec3(self):
		...
		if self.owner.InstalledFams:
			...
			injection = (
				...
				f"\t\t\t\t\tif _installer:\n"
				f"\t\t\t\t\t\t_family_items = _registry.ext.OpFamRegistryExt.getFamilyPopMenuItems(family)\n"
				f"\t\t\t\t\t\t_support_url = _registry.ext.OpFamRegistryExt.getSupportUrl(family)\n"
				f"\t\t\t\t\t\tif _support_url:\n"
				f"\t\t\t\t\t\t\t_items.append('Support')\n"
				f"\t\t\t\t\t\tif _family_items:\n"
				f"\t\t\t\t\t\t\t_dividers.append(_items[-1])\n"
				f"\t\t\t\t\t\t\tfor _fi in _family_items:\n"
				f"\t\t\t\t\t\t\t\t_items.append(_fi.get('label', ''))\n"
				f"\t\t\t\t\t\t\t\tif _fi.get('disabled', False):\n"
				f"\t\t\t\t\t\t\t\t\t_disabled.append(_fi.get('label', ''))\n"
